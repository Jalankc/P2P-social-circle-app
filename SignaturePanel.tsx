...
function wrapWithAnimation(html: string, animation: string): string {
  switch (animation) {
    case 'marquee':
      return `<marquee scrollamount="3" style="max-height:120px">${html}</marquee>`
    case 'fade':
      return `<div style="animation:fadeInOut 3s ease-in-out infinite;max-height:120px">${html}</div>`
    case 'pulse':
      return `<div style="animation:pulse 2s ease-in-out infinite;max-height:120px">${html}</div>`
    case 'slide':
      return `<div style="animation:slideHorizontal 4s ease-in-out infinite;max-height:120px">${html}</div>`
    case 'none':
    default:
      return `<div style="max-height:120px">${html}</div>`
  }
}
...
const DEFAULT_SIGNATURE = `~ [b]\u{1F33F} Super Admin of Willow's Grove[/b] ~
~ 1,247 Posts ~ P2P Seed Collector ~`

const DEFAULT_HTML = `<marquee>\u{1F33F} Willow's Grove ~ Super Admin ~ 1,247 Posts ~</marquee>`
    ...
    <div className="space-y-5">
        ...
        {([
          ...
          <button
            ...
            style={{
              ...
              borderBottom: activeSigTab === tab.key ? '1px solid rgba(45,106,79,0.4)' : '1px solid rgba(45,106,79,0.2)',
              borderRadius: '0px',
            }}
          >
            {tab.label}
      ...
      {activeSigTab === 'html' && (
        <div className="space-y-2">
          ...
          <p className="font-mono text-[10px] text-sp-parchment" style={{ opacity: 0.6 }}>
            JetBrains Mono with terminal green syntax highlighting
          </p>
        </div>
      )}
      ...
      {activeSigTab === 'animation' && (
        ...
        >
          ...
          <p className="font-mono text-[10px] text-sp-parchment" style={{ opacity: 0.6 }}>
            Animation wraps your signature in the appropriate HTML tag. Preview updates live below.
          </p>
        </div>
      )}
